      ...
      <header class="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#f2f2f3] px-10 py-3">
        <div class="flex items-center gap-8">
          ...
          <div class="flex items-center gap-9">
            <a class="text-[#141415] text-sm font-medium leading-normal" href="{{ route('landing') }} ">Beranda</a>
            @auth
            <a href="{{ route('produk.user') }}" class="text-[#141415] text-sm font-medium leading-normal">Produk</a>
            @else
            <a href="{{ route('login') }}" onclick="alert('Silakan login terlebih dahulu untuk melihat produk.')" class="text-[#141415] text-sm font-medium leading-normal">Produk</a>
            @endauth
            <a href="{{ route('tentangkami') }}" class="text-[#141415] text-sm font-medium leading-normal" href="#">Tentang Kami</a>
            <a href="{{ route('pesanan.user') }}" class="text-[#141415] text-sm font-medium leading-normal" href="#">Pemesanan</a>
          </div>
        </div>
        <div class="flex flex-1 justify-end gap-8">
          <!-- Form pencarian -->
          @auth
          <form method="GET" action="{{ route('produk.user') }}" class="mr-6">
          ...
          @endphp
          <form method="GET" action="{{ route('produk.user') }}" onsubmit="{{ $redirectToLogin }}" class="mr-6">
            <label class="flex flex-col min-w-40 !h-10 max-w-64">
              <div class="flex w-full flex-1 items-stretch rounded-xl h-full">
                <div class="text-[#71747a] flex border-none bg-[#f2f2f3] items-center justify-center pl-4 rounded-l-xl border-r-0">
                  <!-- SVG ikon kaca pembesar -->
        ...
        <div class="flex gap-2">
          ...
          <a href="{{ route('login') }}" class="flex items-center gap-2 bg-gray-100 rounded-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">
            <!-- Icon Pengguna (SVG) -->
            <svg class="w-5 h-5 text-gray-600" fill="currentColor" viewBox="0 0 256 256">
              <path d="M230.92,212c-15.23-26.33-38.7-45.21-66.09-54.16a72,72,0,1,0-73.66,0C63.78,166.78,40.31,185.66,25.08,212a8,8,0,1,0,13.85,8c18.84-32.56,52.14-52,89.07-52s70.23,19.44,89.07,52a8,8,0,1,0,13.85-8ZM72,96a56,56,0,1,1,56,56A56.06,56.06,0,0,1,72,96Z"></path>
            </svg>
          ...
          <a href="{{ route('keranjang.tampil') }}" class="relative inline-block">
